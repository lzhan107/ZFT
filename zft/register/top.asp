<table width="970" border="0" cellspacing="2" cellpadding="2">
 <tr>
   <td width="446">&nbsp;</td>
   <td width="510">
    <form action="record.asp" method="post" name="form1">    
    <div class="top">
    <table width="485" border="0" cellspacing="0" cellpadding="0" >
     <tr>
      <td width="80"><a href="register/register.asp"><img src="../image/reg.jpg" width="78" height="26" border="0"></a></td>
      <td width="70" height="13"><span class="fontstyle1">登录名：</span></td>
      <td width="115"><input name="rname" type="text" size="15" /></td>
      <td width="50"><span class="fontstyle1">密码：</span></td>
      <td width="115"><input name="password" type="password" size="15" /></td>
      <td width="55"><img src="../image/button.jpg" width="50" height="26" style="cursor: hand" onclick="javascript:form1.submit();"></td>
     </tr>
    </table>
    </div>   
    </form>
   </td>
 </tr>
 <tr>
  <td colspan="2">
  <div class="link">
   <table width="960" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td height="62" colspan="8"><img src="../image/logo.jpg" width="418zhuce" height="60"></td>
    </tr>
    <tr>
      <td width="43" height="30" background="../image/link/1.gif"></td>
      <td width="50" height="30" align="center" background="../image/link/2.gif"><span class="fontstyle2"><a href="../index.asp" class="channel0">首页</a></span></td>
      <td width="80" height="30" align="center" background="../image/link/3.gif"><span class="fontstyle2"><a href="../about/about_brief.asp" class="channel0">关于中服通</a></span></td>
      <td width="80" height="30" align="center" background="../image/link/3.gif"><span class="fontstyle2"><a href="../series/.asp" class="channel0">服务系列</a></span></td>
      <td width="80" height="30" align="center" background="../image/link/3.gif"><span class="fontstyle2"><a href="../diagnose/diagnose_brief.asp" class="channel0">"自"诊断</a></span></td>
      <td width="74" height="30" align="center" background="../image/link/4.gif"><span class="fontstyle2"><a href="../service/liucheng.asp" class="channel0">我要服务</a></span></td>
      <td width="73" height="30" align="center" background="../image/link/5.gif"><span class="fontstyle2"><a href="../news/.asp" class="channel0">新闻中心</a></span></td>
      <td width="470" align="center"><span class="fontstyle3" style="color: #DC000A;"><strong>中服通专线：025-84879672</strong></span></td>
    </tr>
   </table>
  </div>
  </td>
 </tr>
</table>