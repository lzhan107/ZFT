...
<%  
   ...
   If not(rs.Eof) or not(rs.Bof) Then
	 Response.Write ("<script language=javascript>alert('该新闻已存在，请重新填写！');javascript:history.go(-1)</script>")
	 Response.End
   End If
   Dim sql
	sql="insert into 经济新闻(theme,jdate,jtime,jtext) " & "values('"&theme&"','"&da&"','"&ti&"','"&text&"')"
   Dim Comm
   ...
   Set Comm.ActiveConnection=conn
	   ...
	   Comm.Execute aa,,1
	 If aa=1 Then
	   Response.Redirect "news_jj_.asp"
	   Response.End
	 End If
   rs.close
   Set rs=nothing
